...
export default function Experience() {
  ...
  const generateMeshes = (meshType: string, length: number) =>
    Array.from({ length }, (_, i) => {
      ...
      const position: Vector3 = [(Math.random() - 0.5) * 15, (Math.random() - 0.5) * 15, (Math.random() - 0.5) * 15]
      const rotation: Euler = [Math.random() * Math.PI, Math.random() * Math.PI, 0]
      let geometry
      switch (meshType) {
        case 'torus':
      ...
      return (
        <Float
          ...
          speed={0.75}
        >
          <mesh key={i} position={position} rotation={rotation} scale={[randomSize, randomSize, randomSize]}>
            {geometry}
            {meshNormalMaterial}
          </mesh>
        </Float>
  ...
  return (
    <Canvas camera={{ position: [1, -1, 6.5], fov: 45 }} className='bg-neutral-950'>
